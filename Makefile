...
CAPTURE_FILE = $(BUILD_DIR)/cap_file.pcap

LIBRARIES = -lpcap -lncurses
...
$(ANALYSIS_DIR)/memcheck.out: $(EXEC) $(VALGRIND_DEP)
	@mkdir -p $(@D)
	valgrind --tool=memcheck --leak-check=full --show-leak-kinds=all --track-origins=yes --log-file="$@" ./$(EXEC) -f $(CAPTURE_FILE)

$(ANALYSIS_DIR)/callgrind.out: $(EXEC) $(VALGRIND_DEP)
	@mkdir -p $(@D)
	valgrind --tool=callgrind --callgrind-out-file="$@" ./$(EXEC) -f $(CAPTURE_FILE)
